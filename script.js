...
// The feed is generated by yt-dlp at deploy time, but these values still land
// inside href/src attributes — escape the quote characters that would let a
// malformed title or URL break out of one, and only allow real YouTube links.
...
async function loadLocalYouTubeFeed() {
  ...
  try {
    ...
    youtubeFeed.innerHTML = videos.slice(0, 3).map(video => {
      ...
      const thumb = safeUrl(video.thumbnail);
      const title = escapeText(video.title);
      if (!url) return "";
